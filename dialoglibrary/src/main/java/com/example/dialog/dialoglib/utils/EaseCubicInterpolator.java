...
public class EaseCubicInterpolator implements Interpolator {
    ...
    private final PointF mInputPoint1 = new PointF();

    private final PointF mInputPoint2 = new PointF();
    ...
    public EaseCubicInterpolator(float x1, float y1, float x2, float y2) {
        Log.i("pauseAnimation", "EaseCubicInterpolator");
        mInputPoint1.x = x1;
    ...
    public float getInterpolation(float input) {
        Log.i("getInterpolation", "input: " + input);
        float t = input;
        // 近似求解t的值[0,1]
        for (int i = mLastI; i < ACCURACY; i++) {
            t = 1.0f * i / ACCURACY;
            double x = cubicCurves(t, 0, mInputPoint1.x, mInputPoint2.x, 1);
            if (x >= input) {
                mLastI = i;
                break;
            }
        ...
        double value = cubicCurves(t, 0, mInputPoint1.y, mInputPoint2.y, 1);
